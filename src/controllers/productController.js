...
export const getOneProduct = async (req, res) => {
    ...
    try {
        ...
        console.log({
            message: `Error al obtener el producto id: ${id}: ${error}`,
        });
    }
};
...
export const disableProduct = async (req, res) => {
    ...
    try {
        await Productos.update({ estado: false }, { where: { id } });
        ...
        console.log({
            message: `Error al dar de baja el producto id: ${id}: ${error}`,
        });
    }
};

export const createProduct = async (req, res) => {
    const { titulo, precio, imagen, stock, id_categoria, estado, detalles } =
        req.body;
    try {
        ...
        res.status(201).json({
            message: `Nuevo producto agregado. ID autogenerado: ${nuevoProducto.id}`,
        });
    } catch (error) {
        res.status(500).json({
    ...
    try {
        ...
        if (data.info_disco) {
            ...
            if (!generoDisco) {
                return res
                    .status(400)
                    .json({ message: "Género (disco) no encontrado" });
            }
        ...
        if (data.info_libro) {
            ...
            if (!generoLibro) {
                return res
                    .status(400)
                    .json({ message: "Género (libro) no encontrado" });
            }
        ...
        res.status(200).json({
            message: `Producto id: ${id}, modificado con exito!`,
        });
    } catch (error) {
